...
//
// Two gates: a hotkey command never reports, because its result would land on
// whatever page the user opened next, and a command whose successor is already
// queued stays silent so what is shown belongs to the newest command. Both tests
// and the post are one unit: lcd_task outranks this task and could otherwise
// slip a fresh send between them and inherit this outcome.
...
void lora_task_post_ack(uint8_t ack_state)
{
    ...
    // Only a toggle reports a relay level; anything else (and any state byte an
    // older or unexpected PolyPlug sends) is a plain delivery confirmation
    if (cur_index == 0) {
...
// Every retry is exhausted: stop waiting and report the failure.
// Only the toggle reports it; the other command types show nothing on failure,
// exactly as they did before the relay state was added to the ACK.
...
static void lora_task(void *pvParameters)
{
    ...
    while (1) {
        ...
        // a command sent while this one is in flight waits its turn instead of overwriting it in the queue (where it would be wiped on ACK/give-up)
            ...
            // A toggle whose result will actually be shown carries a marker asking
            // the PolyPlug to report the resulting relay level in its ACK. Older
            // plugs ignore it and reply with the original ACK, which still reads as
            // 'delivered' here. A hotkey's outcome is discarded, so it does not ask
            // and the plug need not hold its ACK while the relay settles.
            if (cur_index == 0 && cur_ui_origin) {
                strcpy(lora_req.cmd.instr, LORA_PCP_INSTR_TOGGLE);
...
{
    // Both queues are created here rather than inside the task, so they exist
    // before lora_task runs. lcd_task is spawned just ahead of this and outranks
    // app_main, but it only touches these once the user opens a LoRa page, which
    // is many button presses after it has yielded and this has run.
