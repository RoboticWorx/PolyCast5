...
// Who the outcome waiting on xLoraReceiptQueue belongs to
// Each page checks these before rendering rather than assuming the result is its own
...
void lcd_lora_create_custom_name(ui_btns_t *ui_btns, ui_menu_t *ui_menu, lora_menu_t *lora_menu)
{
    ...
    if (!ti.active) {
        ...
        ti.title = lora_menu_overwrite ? "Enter new plug name" : "Enter plug name";
        ti.hint = "(Up to 12 characters)";
        ti.prefill = lora_menu_overwrite ? lora_menu->options[lora_menu->index] : NULL;
        ti.lock_until_submit = !lora_menu_overwrite;
...
{    
    ...
    if (xLoraReceiptQueue && xQueuePeek(xLoraReceiptQueue, &receipt, 0) == pdTRUE &&
            receipt_plug == lora_menu->index) { // Ours, not another plug's
        xQueueReceive(xLoraReceiptQueue, &receipt, 0); // Ours: take it
...
void lcd_lora_gpio_subpage(ui_btns_t *ui_btns, ui_menu_t *ui_menu, lora_menu_t *lora_menu)
{
    ...
    // and it is left queued, so returning to the submenu still shows it.
    lora_receipt_t gpio_receipt;
    if (xLoraReceiptQueue && xQueuePeek(xLoraReceiptQueue, &gpio_receipt, 0) == pdTRUE &&
            receipt_plug == lora_menu->index && receipt_cmd == LORA_SUBMENU_GPIO_IDX) {
        xQueueReceive(xLoraReceiptQueue, &gpio_receipt, 0); // Ours: take it
