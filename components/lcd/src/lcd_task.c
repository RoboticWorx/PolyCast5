...
ui_menu_t ui_menu = {
	.options = (const char *[]) {"Bluetooth", "PolyPlug", "ESP32", "Infrared", "Tools",  "Settings", "GPIO", "Wi-Fi"},
	.size = 8,
	.index = 1, // PolyPlug starts in the middle
	.page = HOME_PAGE,
	.lbl_top = NULL,
	.lbl_mid = NULL,
	.lbl_bot = NULL,
...
static void lcd_task(void *pvParameters)
{
	...
	while (1)
	{
		if (xTaskGetTickCount() - btn_timer_last >= btn_timer_interval) {
			...
			else if (ui_menu.page == LORA_GPIO_SUBPAGE) {
			...
			else if (ui_menu.page == BLUETOOTH_HOW_PAGE) {
			...
			else if (ui_menu.page == BLUETOOTH_RENAME_PEER_PAGE) {
