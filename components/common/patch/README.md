...
* Capping the clock speed at 160MHz (device becomes slightly slower).

These workarounds are not required for ESP32-C5s with an eFuse rev of 0.4+.

The patch applies one workaround or the other depending on the build profile. For regular users this is the `Dev` build (allows re-flashing), if you're securing your device from [lock-it-down](https://polycast5.com/blogs/docs/lock-it-down), this will be the `Locked` build.

| Build | PSRAM | Clock, blk rev v0.3 | Clock, v0.4+ |
| --- | --- | --- | --- |
| Dev (`POLYCAST5_SECURE_RELEASE 0`) | plaintext | 240 MHz | 240 MHz |
| Locked (`POLYCAST5_SECURE_RELEASE 1`) | encrypted | 160 MHz | 240 MHz |

*Flash/NVS stay encrypted in both profiles.*
...
v1.0 and older, because of a hardware bug there. The patch extends that exception to dev builds on
every chip revision, and leaves locked builds alone:

* Dev builds: PSRAM pages (`MMU_TARGET_PSRAM0`) never get the sensitive bit. The effect is exactly
  the state IDF already ships on v1.0 silicon: encrypted flash, plaintext PSRAM.
* Locked builds (`CONFIG_SECURE_FLASH_ENCRYPTION_MODE_RELEASE`): unchanged from stock, so PSRAM
  stays encrypted and `app_main` caps the clock instead.
* The anti-fault-injection assert that follows the write matches whichever branch was compiled in.
  Without that it fires on every PSRAM page and the chip resets while mapping PSRAM at boot.
* Flash pages are untouched in both profiles. Flash, NVS and the firmware image at rest stay
  encrypted.

The patch also adds `#include "sdkconfig.h"`, which the header otherwise does not pull in. Without
it `CONFIG_SECURE_FLASH_ENCRYPTION_MODE_RELEASE` is undefined, `#if` silently takes the dev branch,
and a locked build would ship plaintext PSRAM.

### Dev build security consequence
...
**This means if you store sensitive data on here you need to lock down your device ([tutorial](https://polycast5.com/blogs/docs/lock-it-down)) to prevent extraction.** This enables release mode encryption and secure boot, while capping the device at **160MHz (only if your eFuse is rev 0.3)** to secure PSRAM contents.

### Apply

You may need to change the installation path:
...
`git apply --check` prints nothing on success, so say it out loud. Run this before any release
build:

```bash
git -C C:/Espressif/.espressif/v6.0.1/esp-idf apply --check -R C:/Projects/ESP/PolyCast5/components/common/patch/esp32c5-psram-plaintext-mmu_ll.patch && echo "PATCH APPLIED" || echo "PATCH MISSING - PSRAM will be encrypted"
```

A missing patch is not fatal. The firmware reads the live MMU entry at boot and caps itself at
160 MHz, logging a warning and a `WARN` row in the hardware report. You lose 80 MHz, not data.
...
* Cap the DFS ceiling at 160 MHz on affected units (`POLYCAST5_CPU_240_MIN_EFUSE_BLK_REV` in
  ...
  is unacceptable.
* Make the dev branch lot-conditional too: replace `target != MMU_TARGET_PSRAM0` with
  `!(target == MMU_TARGET_PSRAM0 && efuse_hal_blk_version() < 4)` and mirror it in the assert.
  Dev builds on v0.4 units would then keep encrypted PSRAM at 240 MHz, as they ran for weeks.
