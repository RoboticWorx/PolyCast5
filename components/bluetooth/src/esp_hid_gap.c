...
extern esp_err_t bluetooth_nvs_get_preferred_peer(ble_addr_t *out, bool *found);
...
esp_err_t esp_hid_ble_gap_adv_start(void)
{
    ...
    // If we're already advertising, do nothing
    ...
    adv.conn_mode = BLE_GAP_CONN_MODE_UND; // UNDIRECTED
    ...
    if (found && (pref.type == BLE_ADDR_PUBLIC || pref.type == BLE_ADDR_RANDOM)) {
        ...
        if (rc != 0) {
            ESP_LOGE(TAG, "wl_set failed rc=%d", rc);
            return rc;
        }

        // Require whitelist for BOTH scan and connection
        adv.filter_policy = BLE_HCI_ADV_FILT_BOTH;

        // Use privacy-friendly own address
        rc = ble_hs_id_infer_auto(1 /* allow privacy */, &own_addr_type);
        if (rc != 0) {
            MODLOG_DFLT(ERROR, "error infer own addr rc=%d\n", rc);
    ...
    // No preferred: general open advertising
    adv.filter_policy = BLE_HCI_ADV_FILT_DEF; // Default

    rc = ble_hs_id_infer_auto(1 /* allow privacy */, &own_addr_type);
    if (rc != 0) {
        MODLOG_DFLT(ERROR, "error enabling infer_auto; rc=%d\n", rc);
        return rc;
    }
